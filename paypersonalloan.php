...
if(isset($_POST['pay_to']) and isset($_POST['payment'])){
	...
	if($pay_to!='' && $payment!=''){
		@mysqli_query($con,"INSERT INTO `transaction`(`pay_to`, `pay_amount`, `date`, `type`, `store_id`) VALUES ('$pay_to','$payment','$date','loan_personal','1')");
					...
					<td><div class="select_style"><select id="dropdown" name="pay_to" onchange="find_pay_to(this.value)">
						...
						<?php
							...
							while($row = mysqli_fetch_array($sql)){
								echo '<option value="'.$row['id'].'">'.$row['name'].'('.$row['payable_amount'].')'.'</option>';
							}
						?>
