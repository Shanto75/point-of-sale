...
<?php include("includes/header.php"); 
	...
	$whereclass2 .= "and personid = '$customer' and type='supplier'";
	}else{
		$whereclass2 =" and type='supplier'";
	}
	$datarr= array();

	
	$sql2 = mysqli_query($con,"SELECT `p_id`, `date`, `supplier`, `product_name`, `payment`, `payable`,`type` FROM `purchase` where 1 $whereclass and count=1 and type='purchase' ");
	while($row2 = mysqli_fetch_array($sql2)){
		
		 $customerName= mysqli_fetch_array(mysqli_query($con, "SELECT name FROM personinformation WHERE type='supplier' AND id='".$row2['supplier']."'"));
	...
	while ($r4 = mysqli_fetch_array($sql4)) {
		 ...
		 $customerName= mysqli_fetch_array(mysqli_query($con, "SELECT name FROM personinformation WHERE type='supplier' AND id='".$r4['personid']."'"));
	...
	$sql3 = mysqli_query($con,"select * from transaction where status = 'due' and type='purchase' $whereclass1");
	while($r = mysqli_fetch_array($sql3)){

		$customerName= mysqli_fetch_array(mysqli_query($con, "SELECT name FROM personinformation WHERE type='supplier' AND id='".$r['name']."'"));
			$datarr[] = array($r['date'],$customerName['name'],'Supplier Due payment',$r['pay_amount'],'','');	
	}
...
<div class="area">
      <div class="panel-head">Supplier Ledger Report</div>
      <div class="panel">
   ...
   <table width="400px" class="tab form" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td width="5%"><select name="customer">
					...
					<?php 
						$bquery = mysqli_query($con,"select * from personinformation where type = 'supplier' and name!=''");
...
<div style="overflow:hidden;padding:10px;margin-bottom:10px;text-align: right;" >	
		<a  href="generate_supplier_ledger.php?from=<?= $f ?>&to=<?= $t ?>&customer=<?= $_GET['customer']==''?'':$_GET['customer'] ?>" class="btn-add">Export/Print</a>
</div>
...
?>
		...
		<tfoot>
			<tr>
				...
				<th></th>
				<th></th>
				<td>Dr <?= array_sum($out); ?></td>
				<td>Cr <?= array_sum($in); ?></td>
