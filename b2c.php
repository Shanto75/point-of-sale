...
if(isset($_POST['bank'])){
	...
	if(!isset($_POST['id'])){
		@mysqli_query($con,"INSERT INTO `transfer`(`date`, `bank_name`, `ac_name`, `ac_number`, `amount`, `comments`, `type`,bankid) VALUES ('$date','$bank_name','$acname','$acnumber','$amount','$comments','b2c','$bank')");
		$msg = 'Data Inserted Successfully!';
	}else{
		$id = $_POST['id'];
		@mysqli_query($con,"UPDATE `transfer` SET `date`='$date',`bank_name`='$bank_name',`ac_name`='$acname',`ac_number`='$acnumber',`amount`='$amount',`comments`='$comments' WHERE 1 and id = '$id'");
...
?>
	<div class="area">
		<div class="panel-head">Bank to Cash Details</div>
		<div class="panel">
								<?php 
		if($msg!=''){
    ...
    background-color: #DFF2BF;border:1px solid #ddd;padding:10px;border-radius:5px;text-align:center;font-size:15px;">'.$msg.'</p>';
		...
		?>
		<div class="btn">	
		<a onclick="return emptyform();" href="javascript:;"id="modal-launcher" class="btn-add">+ Add Bank to Cash</a></div>
	<!--Add-->	
		<div id="modal-background"></div>
<div id="modal-content">
    <button id="modal-close">Close Window</button>
    <div class="modalform">
    	Add a Bank to Cash<hr/>
...
<!--View-->
<?php 
$sql = @mysqli_query($con,"select * from transfer where 1 and type='b2c'");
?>
    ...
    <tr>
            ...
            <a href="b2c.php?id=<?= $row['id'] ?>&del=del"id="" class="delete">Delete</a></div>
            </td>
    </tr>
	<?php } ?>
    </tbody>
