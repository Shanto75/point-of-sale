...
function checkproductdetails(val){
	...
	$.post('check_customer_details.php', {productid: val},
	function (data) {
	...
	if(isset($_POST['id'])){
		...
		mysqli_query($con,"UPDATE `personinformation` SET `name`='$name',`address`='$address',`phone`='$phone',`email`='$email',`comments`='$comments' WHERE 1 and id = '$id'");
			...
			if($_POST['opening_balance']!=''){
                ...
                if(!empty($pdata)){
				@mysqli_query($con, "UPDATE openingbalance SET amount='$openingbalance' WHERE type='customer' AND personid='$id'");
				}else{
					 @mysqli_query($con, "INSERT INTO `openingbalance`(`personid`, `type`, `amount`,`date`)VALUES('$id','customer','$openingbalance','$date')");
				}
		...
		@mysqli_query($con,"INSERT INTO `personinformation`(`name`, `address`, `phone`, `email`, `company_name`, `type`, `comments`, `store_id`) VALUES ('$name','$address','$phone','$email','','customer','$comments','$store_id')") or die();
		     ...
		     @mysqli_query($con, "INSERT INTO `openingbalance`(`personid`, `type`, `amount`,`date`)VALUES('$personId','customer','$openingbalance','$date')");
...
?>
    ...
    background-color: #DFF2BF;border:1px solid #ddd;padding:10px;border-radius:5px;text-align:center;font-size:15px;">'.$msg.'</p>';
		...
		?>
		<div class="btn">	
		<a onclick="return emptyform();" href="javascript:;"id="modal-launcher" class="btn-add">+ Add Customer</a></div>
	<!--Add-->	
...
<div id="modal-content">
    ...
    <div class="modalform">
    	...
    	<td><input type="text" id="name" name="name" placeholder="Enter Customer Name"></td>
    	</tr>
		<tr>
    	<td align="right">Customer Opening  Balance</td>
    	<td><input type="text" id="opening_balance" name="opening_balance"></td>
    	</tr>
    	<tr>
    	<td align="right">Customer Phone</td>
    	<td><input type="text" id="phone" name="phone"  placeholder="Enter Customer Phone"></td>
    	</tr>
    	<tr>
    	<td align="right">Customer Email</td>
    	<td><input type="text" id="email" name="email" placeholder="Enter Customer Email"></td>
    	</tr>
    	<tr>
    	<td valign="top" style="padding-top:20px;" align="right">Customer Address</td>
    	<td><textarea placeholder="Enter Customer Address" id="address" name="address" rows="5" cols="20"></textarea></td>
    	</tr>
    	<tr>
    	<td valign="top" style="padding-top:20px;" align="right">Customer Comments</td>
...
<?php
	...
	$sql = mysqli_query($con,"select * from personinformation where type = 'customer'");
