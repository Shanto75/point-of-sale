...
function checkproductdetails(val){
	...
	$.post('check_brand_details.php', {productid: val},
	function (data) {
...
function view_earning(val){
	$('#datatable tr').remove(); 
	$.post('view_brand.php', {productid: val},
	function (data) {		
...
if(isset($_POST['brand']) and $_POST['brand']!=''){
	...
	if(!isset($_POST['id'])){
		$catchk = @mysqli_fetch_array(mysqli_query($con,"select * from brand where name = '$brand'"));
		if(empty($catchk)){
			@mysqli_query($con,"INSERT INTO `brand`(`brand_name`, `comments`, `store_id`) VALUES ('$brand','$comments','1')");
			$msg = 'Brand Inserted!';
		}
	}else{
		$id = $_POST['id'];
		@mysqli_query($con,"UPDATE `brand` SET `brand_name`='$brand',`comments`='$comments' WHERE 1 and id = '$id'");
		$msg = 'Brand Updated!';
	}
}
...
?>
	<div class="area">
		<div class="panel-head">Brand Details</div>
		<div class="panel">
		<?php 
		if($msg!=''){
    ...
    background-color: #DFF2BF;border:1px solid #ddd;padding:10px;border-radius:5px;text-align:center;font-size:15px;">'.$msg.'</p>';
		...
		?>
		<div class="btn">	
		<a onclick="return emptyform();" href="javascript:;"id="modal-launcher" class="btn-add">+ Add Brand</a></div>
	<!--Add-->	
...
<div id="modal-content">
    ...
    <div class="modalform">
    	...
    	<td><input type="text" name="brand" id="brand" placeholder="Enter Brand Name"></td>
    	</tr>
    	<tr>
    	<td valign="top" style="padding-top:20px;" align="right">Brand Comments</td>
    	<td><textarea placeholder="Enter Brand Comments" name="comments" id ="comments" rows="5" cols="20"></textarea></td>
    	</tr>
    	<tr>
...
<?php 
    ...
    <tr>
            ...
            <a onclick="return confirm('Are you sure?')" href="brand.php?id=<?= $row['id'] ?>&del=del"id="" class="delete">Delete</a></div>
            </td>
    </tr>
	<?php } ?>
    </tbody>
