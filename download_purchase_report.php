...
require_once('tcpdf/tcpdf.php');
	...
	$totalsale = mysqli_fetch_array(mysqli_query($con,"select sum(subtotal) as total from purchase where `delete` !='1' and type='purchase' and register_mode='purchase' $whereclass")); 
	$totalrec = mysqli_fetch_array(mysqli_query($con,"select sum(payment) as total from purchase where `delete` !='1' and type='purchase' and register_mode='purchase' $whereclass")); 
	$totaldes = mysqli_fetch_array(mysqli_query($con,"select sum(dis_amount) as total from purchase where `delete` !='1' and type='purchase' and register_mode='purchase' $whereclass")); 
	$totalout = mysqli_fetch_array(mysqli_query($con,"select sum(balance) as total from purchase where `delete` !='1' and type='purchase' and register_mode='purchase' $whereclass")); 
...
while($row=mysqli_fetch_array($sql)){
	...
	if($row['balance']>1){
		$due = @mysqli_fetch_array(@mysqli_query($con,"select sum(pay_amount)as total from transaction where type='purchase' and status='due' and name = '".$row['supplier']."'"));
...
$pdf->SetHeaderData('', '','Purchase Report', $pdf_header_strng, array(0,64,255), array(0,64,128));
