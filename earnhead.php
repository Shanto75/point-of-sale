...
function checkproductdetails(val){
	...
	$.post('check_earning_head.php', {productid: val},
	function (data) {
...
function view_earning(val){
	$('#datatable tr').remove(); 
	$.post('view_earning.php', {productid: val},
	function (data) {		
...
if(isset($_POST['head']) and $_POST['head']!=''){
	...
	if(!isset($_POST['id'])){
		mysqli_query($con,"INSERT INTO `earning_head`(`head`, `store_id`, `comments`) VALUES ('$head','1','$comments')");
		$msg =  'Data inserted Successfully!';
	}else{
		$id = $_POST['id'];
		mysqli_query($con,"UPDATE `earning_head` SET `head`='$head',`comments`='$comments' WHERE 1 and id = '$id'");
		$msg =  'Data Updated Successfully!';
	}
}
if(isset($_GET['del']) and $_GET['id']!=''){
	$id = $_GET['id'];
	@mysqli_query($con,"delete from earning_head where id = '$id'");
	$msg = 'Data deleted successfully!';
}
?>
	<div class="area">
		<div class="panel-head">Earning Head Details</div>
		<div class="panel">
		<?php 
		if($msg!=''){
    ...
    background-color: #DFF2BF;border:1px solid #ddd;padding:10px;border-radius:5px;text-align:center;font-size:15px;">'.$msg.'</p>';
		...
		?>
		<div class="btn">	
		<a onclick="return emptyform();" href="javascript:;"id="modal-launcher" class="btn-add">+ Add Earning Head</a></div>
	<!--Add-->	
		<div id="modal-background"></div>
<div id="modal-content">
    <button id="modal-close">Close Window</button>
    <div class="modalform">
    	Add a Earning Head<hr/>
    	...
    	<tr>
    	<td valign="top" style="padding-top:20px;" align="right">Earning Head Comments</td>
