...
<?php include("includes/header.php"); 
	...
	$sql = mysqli_query($con,"SELECT `date`, `earning_head`, sum(`amount`)as amount FROM `earning` WHERE 1 $whereclass group by date");
	
	$total = @mysqli_fetch_array(mysqli_query($con,"SELECT `date`, `earning_head`, sum(`amount`)as amount FROM `earning` WHERE 1 $whereclass")); 
...
function view_invoice_details(val){
	...
	$('#datatable tr').remove(); 
	$.post('view_earn_ajax.php', {productid: val},
	function (data) {		
...
<div class="area">
      <div class="panel-head">Earnings Report</div>
      <div class="panel">
