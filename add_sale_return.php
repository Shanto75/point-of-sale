                        ...
                        if (data.sell != undefined){
							...
							if ($("#" + item).length > 0)
							{
								...
								$("#item").val('');
									...
									$('<tr bgcolor="#f3f3f3" id=' + item + '><td class="center tdpad"><input type=hidden value=' + item + ' id=' + item + 'id ><input type=hidden value=' + product_code + ' name="pcode[]" ><input type=button value="Delete" id=' + item + ' onclick=reduce_balance("' + item + '");$(this).closest("tr").remove(); ></td><td class="center tdpad"><input type="text" size="8" value="'+product_code+'"/></td><td class="center tdpad"><input name="stock_name[]" id=' + item + 'st value="'+ code +'" class="center tdpad" style="border:none" type="text"/></td><td class="center tdpad"><input onkeypress="return numbersonly(event);" onkeyup=quantityupdate("' + item + '") name=quty[] value=' + quty + ' id=' + item + 'q class="center tdpad" size="5" type="text"/></td><td class="center tdpad"><input name=sell[] value=' + sell + ' id=' + item + 's onkeyup=priceupdate("' + item + '") class="center tdpad" size="5" type="text"/></td><td class="center tdpad"><input name=stock[] readonly="readonly" value=' + disc + ' id=' + item + 'p class="center tdpad" size="5" type="text"/></td><td class="center tdpad"><input name=jibi[] readonly="readonly" value=' + total + ' id=' + item + 'to class="center tdpad" size="8" type="text"/><input type=hidden name=total[] id=' + item + 'my_tot value=' + main_total + '></td></tr>').fadeIn("slow").appendTo('#item_copy_final');
								$("#item").val('');
								$('#empty').hide(); 
...
<?php 
    ...
    $autoid1 = "SL-" . $max . "";
        ...
        $customerId = '';
        $chkcustomer = mysqli_fetch_array(mysqli_query($con,"select * from personinformation where type = 'customer' and name = '$supplier' and phone='$contact'"));
        if(empty($chkcustomer)){
            ...
            $chkcustomerId = mysqli_fetch_array(mysqli_query($con, "SELECT id FROM personinformation WHERE type='customer' AND name='$supplier' AND phone='$contact'"));
...
background-color: green;border:1px solid #ddd;padding:10px;border-radius:5px;text-align:center;font-size:15px;">'.$msg.'</p>';
            ...
            <table border="0" cellspacing="0" cellpadding="0" id="item_copy_final">
                ...
                <tr bgcolor="" class="scan">
					...
					<td class="center" width="10%">Code</td>
...
<tr bgcolor="#f3f3f3" id="<?= $pdetails['stock_id'] ?>">
    ...
    <td class="center tdpad">
        <input type=hidden value="<?= $pdetails['stock_id'] ?>" id="<?= $pdetails['stock_id'] ?>id" ><input type=button value="Delete" id="<?= $pdetails['stock_id'] ?>" onclick=reduce_balance("<?= $pdetails['stock_id'] ?>");$(this).closest("tr").remove(); >
    </td>
	<td>
		<input type="text" size="8" name="pcode[]" value="<?= $row['pcode'] ?>" >
	</td>
    ...
    <td class="center tdpad">
        <input name=sell[] value="<?= $row['sale_price'] ?>" id="<?= $pdetails['stock_id'] ?>s" onkeyup=priceupdate("<?= $pdetails['stock_id'] ?>") class="center tdpad" size="5" type="text"/>
    </td>
...
?>
        ...
        <div class="prodright left fix">
            <div class="customer_details fix">
                <table>
<?php 
//$find_cus = mysqli_fetch_array(mysqli_query($con,"select * from personinformation where type = 'customer' and name = '$custo'"));

$find_cus = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM personinformation WHERE type='customer' AND id='$custo'"));
?>
                    <tr>
                        <td>Customer: </td>
                        <td><input name="supplier" placeholder="Start typing customer name.." type="text" id="supplier"
                                   maxlength="200" value="<?= $find_cus['name'] ?>" class="form-control" style="width:250px" /></td>
                    </tr>
                    <tr><td>Address: </td>
                    ...
                    <tr>
                        ...
                        <td><input name="contact" placeholder="" type="text" id="contact1"
                                   ...
                                   onkeypress="return numbersonly(event)" value="<?= $find_cus['phone'] ?>" style="width:250px "/></td>
                    </tr>
                </table>
            </div>
            <hr />
            <div class="totalamount fix">
                <table>
                    ...
                    <tr>
                        ...
                        <td width="50%"><input type="text"
                                                   onkeypress="return numbersonly(event);"
                                                   class="form-control"
                                                   id="total_amount" value="<?= $payable ?>" style="text-align:right;width: 120px"></td>
                    </tr>
                </table>
                <?php 
                ...
                <table width="280">
                    <tr>
                        <td align="right"><input type="submit" onclick="return chkdata();" name="submit11" value="Salse Return" /><td>
                    </tr>
                </table>
            </div>
            </form>
