<?php include("includes/header.php"); 
	...
	$sql = mysqli_query($con,"select * from purchase where `delete` !='1' and type='purchase' and register_mode='return' $whereclass group by p_id");
	
	$totalsale = mysqli_fetch_array(mysqli_query($con,"select sum(subtotal) as total from purchase where `delete` !='1' and type='purchase' and register_mode='return' $whereclass and count = 1")); 
	$totalrec = mysqli_fetch_array(mysqli_query($con,"select sum(payment) as total from purchase where `delete` !='1' and type='purchase' and register_mode='return' $whereclass and count = 1")); 
	$totaldes = mysqli_fetch_array(mysqli_query($con,"select sum(dis_amount) as total from purchase where `delete` !='1' and type='purchase' and register_mode='return' $whereclass and count = 1")); 
	$totalout = mysqli_fetch_array(mysqli_query($con,"select sum(balance) as total from purchase where `delete` !='1' and type='purchase' and register_mode='return' $whereclass and count = 1")); 
...
<div class="area">
      <div class="panel-head">Product Purchase Return Report</div>
      <div class="panel">
...
<div style="overflow:hidden;padding:10px;margin-bottom:10px;text-align: right;" >	
		<a  href="generate_purchase_return_report.php?from=<?= $f ?>&to=<?= $t ?>" class="btn-add">Print</a>
...
while($row=mysqli_fetch_array($sql)){
	...
	$customerName = mysqli_fetch_array(mysqli_query($con, "SELECT name FROM personinformation WHERE type='supplier' AND id='".$row['supplier']."'"));
?>
		<tr>
...
<?php } ?>
	...
	<table>
		<tr>
			<th align="left">Total Purchase Return: </th>
