...
require_once('tcpdf/tcpdf.php');
	...
	$sql = mysqli_query($con,"select * from purchase where `delete` !='1' and (type='sale' or type='wholesale') and register_mode='sale' $whereclass group by p_id");
	
	$totalsale = mysqli_fetch_array(mysqli_query($con,"select sum(subtotal) as total from purchase where `delete` !='1' and (type='sale' or type='wholesale') and register_mode='sale' $whereclass")); 
	$totalrec = mysqli_fetch_array(mysqli_query($con,"select sum(payment) as total from purchase where `delete` !='1' and (type='sale' or type='wholesale') and register_mode='sale' $whereclass")); 
	$totaldes = mysqli_fetch_array(mysqli_query($con,"select sum(dis_amount) as total from purchase where `delete` !='1' and (type='sale' or type='wholesale') and register_mode='sale' $whereclass")); 
	$totalout = mysqli_fetch_array(mysqli_query($con,"select sum(balance) as total from purchase where `delete` !='1' and (type='sale' or type='wholesale') and register_mode='sale' $whereclass")); 
	$due1= @mysqli_fetch_array(@mysqli_query($con,"select sum(pay_amount)as total from transaction where type='sale' and status='due'"));
...
while($row=mysqli_fetch_array($sql)){
	...
	$data .='<td>'.$row['bill_no'].'</td>
			<td>'.ucwords($row['supplier']==''?'Guest Supplier':$row['supplier']).'</td>
			<td>'.number_format((float)$row['payable'], 2, '.', '') .' TK</td>
			<td>'. number_format((float)$row['dis_amount'], 2, '.', '') .' TK</td>
			<td>'. number_format((float)$paymnt, 2, '.', '') .' TK</td>
			<td>'. number_format((float)$bal, 2, '.', '') .' TK</td>
...
$pdf->SetHeaderData('', '','Sales Report', $pdf_header_strng, array(0,64,255), array(0,64,128));
