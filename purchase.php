...
function chkdata(){
	...
	with(frm){
		...
		if(parseFloat(payable.value)<parseFloat(payment.value)){					
...
$ms = '';
	...
	$autoid1 = "ST-" . $max . "";
		...
		if($mode=='bkash'){
			...
			$check_date= modifydate($_POST['check_date'],'-','/');
		}
		else{
			$mode_value = '';
		}
		...
		header("Location:add_purchase_print.php?sid=$p_id");
		}else{
...
?>	
<div class="area" >
	<div class="panel-head">Purchase Product</div>
	<div class="panel fix">
		...
		<div class="prodright left fix">
			<div class="customer_details fix">
				<table>
					<tr>
						<td>Date: </td>
						<td><input name="date" type="text" id="date" value="<?= date('d/m/Y'); ?>"
								   maxlength="200" class="form-control datepick" style="width:250px" /></td>
					</tr>
					<tr>
						<td>Supplier: </td>
