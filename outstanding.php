...
$approval='success';
	...
	mysqli_query($con,"INSERT INTO `transaction`(`pay_amount`, `date`, `type`, `store_id`, `name`,status,method,bank,check_number,check_date,`approval`) VALUES ('$pay','$date','$type','1','$name','$status','$payment_type','$bank_name','$check_number','$check_date','$approval')");
	$msg .= 'Amount has been paid';
		$data = urlencode(serialize($_POST));
	header('Location:print_slip_rcv.php?data='.$data);exit;
}
?>
	<div class="area">
		<div class="panel-head">Outstanding Details</div>
		<div class="panel">
		<?php 
		if($msg!=''){
...
<div id="modal-content">
    ...
    <div class="modalform">
		...
		<table class="tab">
			...
			<input type="hidden" value="Customer Payment" name="slip"/>
...
<!--Edit-->
<?php 
	$sql = @mysqli_query($con,"SELECT DISTINCT `p_id`, `supplier`, sum(`payment`)as payment, sum(`balance`)as balance, sum(`payable`)as payable FROM `purchase` WHERE 1  and (type='sale' or type='wholesale') and register_mode = 'sale' and count = '1' and approve!='pending' group by `supplier`");
?>
...
?>
    <tr>
            ...
            <td>
				<a onclick="return branch_view('<?= $row['supplier'] ?>','<?= number_format((float)$row['payable'], 2, '.', '') ?>','<?= number_format((float)$due, 2, '.', '') ?>','<?= number_format((float)$paid, 2, '.', '') ?>');" href="javascript:;"id="modal-launcher" class="view">Receive</a>
            </td>
    </tr>
<?php } ?> 
    </tbody>
